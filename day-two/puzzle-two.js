...
sequences.forEach((sequence) => {
  for (let i = sequence.start; i <= sequence.end; i++) {
    ...
    if (!factorsMap.get(toString.length)) {
      factorsMap.set(toString.length, findFactors(toString.length));
    }
    const factors = factorsMap.get(toString.length);
    for (let j = 0; j < factors.length; j++) {
      const regex = new RegExp(`.{${factors[j]}}`, "g");
      const substrings = toString.match(regex);
      if (substrings.length === 1) continue;
      const mismatches = substrings.filter(
        (substring) => substring !== substrings[0]
      );
